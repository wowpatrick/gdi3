...
testEscapeSeriesForPoint(complex float *c, int maxIterations, complex float *last)
{
    ...
    int fullcondition = {
        ...
        && (cBetrag(last[0]) <= RADIUS)
        && (cBetrag(last[1]) <= RADIUS)
        && (cBetrag(last[2]) <= RADIUS)
        && (cBetrag(last[3]) <= RADIUS)
    };

    __m128 real_last_m128, imag_last_m128;
    
    
    // Run as long as we have packs of 4 vaules that are not in the mandelbrot set
    while (fullcondition){
...
//        float *imag_last = (float*) memalign(4 * sizeof(float), 16); // align to 16-byte for SSE
 //       float *real_last = (float*) memalign(4 * sizeof(float), 16); // align to 16-byte for SSE
        ...
        // Calculate last and increment the number of needed iterations
        for (int j = 0; j < 4; j++){
    ...
    // Exception for the case that we don't have 4 values to calculate (edge condition)
    for (int i = 0 ; i < 4; i++){
