...
float _Complex cSqr(float _Complex a){
float real = crealf(a);
float imag = cimagf(a);
...
float _Complex cAdd(float _Complex a, float _Complex b){
...
{
	int iteration = 0;
	while (cBetrag(*last) <= RADIUS && iteration < maxIterations){
		*last = cAdd(cSqr(*last),c);
		iteration ++;
	}
	if (iteration < maxIterations){
		float mu = log ( log ( cBetrag(*last)) / log(2)) / log(2);
